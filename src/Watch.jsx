...
toast.error(error.response.data.message, {
  ...
  // ✅ Fetch all watches from backend
  useEffect(() => {
  ...
  // ✅ Update selected watch & related watches when data or id changes
  useEffect(() => {
    if (watchdetails.length > 0) {
      const selectedWatch = watchdetails.find((watch) => String(watch._id) === String(id));
      if (selectedWatch) {
        setwatchdata(selectedWatch);

        // ✅ Find related watches
        const related = watchdetails.filter(
          (watch) =>
            watch.Gender === selectedWatch.Gender &&
            watch.brand === selectedWatch.brand &&
            String(watch._id) !== String(id)
        );
  ...
  const navigatetorelatedproduct = (watchId) => {
    setquantity(0);
    navigate(`/watch/${watchId}`);
  };

  // ✅ Pagination for related watches
  const handlePrev = () => {
  ...
  return (
    <div className="px-4 py-8 md:py-16 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        ...
        <div className="px-2 md:px-4">
          ...
          <div className="mt-6 flex flex-col gap-3">
            <button
              onClick={() => additemstocart(watchdata, quantity)}
              className="w-full cursor-pointer h-[50px] bg-orange-600 text-white rounded shadow hover:bg-orange-700 transition hover:underline"
            >
              Add to Cart
            </button>
      ...
      <div className="flex items-center w-full mt-15 px-4 sm:px-6 md:px-8 lg:px-10">
        <div className="text-2xl font-normal whitespace-nowrap">You Might Also Like</div>
        ...
        <div className="grid grid-rows-1 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          ...
          {relatedwatches
            ...
            .map((watch) => (
              <div
                key={watch._id}
                onClick={() => navigatetorelatedproduct(watch._id)}
